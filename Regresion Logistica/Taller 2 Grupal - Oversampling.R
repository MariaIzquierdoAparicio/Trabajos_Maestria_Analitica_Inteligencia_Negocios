...
# 6. Transformación de datos------
  ...
  ## 6.3. Balanceo con Oversampling ------
   ### Dataset Train -----
    # I. Creación de Semilla
    semilla = 611101
    set.seed(semilla) #Se deja la semilla para que el muestreo sea replicable
    
    data_uno <- data[data$resultado == 1, ]
    data_cero<- data[data$resultado == 0, ]
    
    # Creación dataset test (80% de ceros)
    sample = sample.int(nrow(data_cero), round(.8*nrow(data_cero)))
    data_cero_train = data_cero[sample, ]
    data_cero_rest = data_cero[-sample, ]
    
    # Creación dataset test (80% de unos)
    sample = sample.int(nrow(data_uno), round(.8*nrow(data_uno)))
    data_uno_train = data_uno[sample, ]
    data_uno_rest = data_uno[-sample, ]
    
    # Calcular la diferencia entre el número de observaciones en cada grupo
    diferencia <- nrow(data_cero_train) - nrow(data_uno_train)
    
    # Realizar oversampling solo en el grupo con "resultado" igual a 1 para igualar las clases
    grupo_1_oversampled <- data_uno_train[sample(1:nrow(data_uno_train), diferencia, replace = TRUE), ]
    data_uno_train = rbind(data_uno_train, grupo_1_oversampled)
    
    # Combinar los grupos equilibrados
    data_train <- rbind(data_uno_train, data_cero_train)
    
    ### II. Dataset Test -----
    data_test <-  rbind(data_uno_rest, data_cero_rest)
...
COMP_table$Iteración[N]=N
COMP_table$Modelo[N] = c("β + Factura_online + Plan_de_datos + facturación + mora + minutos + Edad + Antiguedad_contrato_años + hombres + mujeres")
...
COMP_table$Iteración[N]=N
COMP_table$Modelo[N] = c("β + Factura_online + Plan_de_datos + facturación + mora + minutos + Edad + Antigüedad_contrato_años + hombres + mujeres + mora_sqr + Edad_sqr + Antigüedad_contrato_años_sqr + Edad_ln + Antigüedad_Equipo_inversa + Edad_inversa + Antigüedad_contrato_años_inversa")
...
COMP_table$Iteración[N]=N
COMP_table$Modelo[N] = c("β + Factura_online + Plan_de_datos + facturación + mora + minutos + Edad + Antigüedad_contrato_años + hombres + mujeres + mora_sqr + Edad_sqr + Antigüedad_contrato_años_sqr + Edad_ln + Antigüedad_Equipo_inversa + Edad_inversa + Antigüedad_contrato_años_inversa + mora_edad")
...
modelo7 <- stepAIC(glm(resultado ~ ., data = data_train, family = binomial), direction = "both")
summary(modelo7)

COMP_table$Iteración[N]=N
COMP_table$Modelo[N] = c("β + Factura_online + Plan_de_datos + facturación + mora + Edad + Antigüedad_contrato_años + hombres + mujeres + mora_sqr + Edad_sqr + Antigüedad_contrato_años_sqr + minutos_ln + Edad_ln + Antigüedad_Equipo_inversa + Edad_inversa + Antigüedad_contrato_años_inversa + mora_edad + comunidad_baja + comunidad_media")
...
N=8
### I. Compute the stepwise logistic regression ----
modelo8 <- glm(resultado ~ Factura_online + Plan_de_datos + facturación + mora + Edad + Antigüedad_contrato_años + hombres + mujeres + mora_sqr + Edad_sqr + Antigüedad_contrato_años_sqr + minutos_ln + Edad_ln + Antigüedad_Equipo_inversa + Edad_inversa + Antigüedad_contrato_años_inversa + mora_edad + comunidad_baja + comunidad_media + minutos_facturacion, data = data_train, family = binomial)
summary(modelo8)

COMP_table$Iteración[N]=N
COMP_table$Modelo[N] = c("β + Factura_online + Plan_de_datos + facturación + mora + Edad + Antigüedad_contrato_años + hombres + mujeres + mora_sqr + Edad_sqr + Antigüedad_contrato_años_sqr + minutos_ln + Edad_ln + Antigüedad_Equipo_inversa + Edad_inversa + Antigüedad_contrato_años_inversa + mora_edad + comunidad_baja + comunidad_media + minutos_facturacion")
...
COMP_table$Iteración[N]=N
COMP_table$Modelo[N] = c("β + Factura_online + Plan_de_datos + facturación + mora + minutos + Edad + hombres + mujeres + mora_sqr + Edad_sqr + Antigüedad_contrato_años_sqr + minutos_ln + Edad_ln + Antigüedad_Equipo_inversa + minutos_inversa + Edad_inversa + Antigüedad_contrato_años_inversa + mora_edad + comunidad_baja + comunidad_media + ME_Minutos_BirthYear + ME_duracion_BirthYear")
...
coeficientes<-modelo2$coefficients
...
probtest<-predict(modelo2,newdata = data_test,type='response')
prontest<-ifelse(probtest > 0.5,1,0)

conftest<-confusionMatrix(as.factor(prontest),data_test$resultado, positive = "1")
conftest$table
conftest$byClass

##nuevo sobre precision
cartllev<-cbind(data_test,probtest)

corte<-0.15
cartllev$pronostico<-ifelse(cartllev$probtest>corte,1,0)
cartllev$error<-cartllev$resultado-cartllev$pronostico
cartllev$error = as.factor(cartllev$error)
probtest1 = ggplot(cartllev,aes(probtest,resultado, color=error))+
            geom_point()+
            geom_vline(xintercept=corte)+
            labs(title = "Precisión y Exhaustividad",
                 subtitle = "Punto de Corte = 0.15",
                 x = "probtest", 
                 y = "Resultado")+
            theme_calc()+scale_fill_calc()
probtest1
...
cartllev$error<-cartllev$resultado-cartllev$pronostico
cartllev$error = as.factor(cartllev$error)
probtest2 = ggplot(cartllev,aes(probtest,resultado, color=error))+
  geom_point()+
  geom_vline(xintercept=corte)+
  labs(title = "Precisión y Exhaustividad",
       subtitle = "Punto de Corte = 0.25",
       x = "probtest", 
       y = "Resultado")+
  theme_calc()+scale_fill_calc()
probtest2
...
cartllev$error<-cartllev$resultado-cartllev$pronostico
cartllev$error = as.factor(cartllev$error)
probtest3 = ggplot(cartllev,aes(probtest,resultado, color=error))+
  geom_point()+
  geom_vline(xintercept=corte)+
  labs(title = "Precisión y Exhaustividad",
       subtitle = "Punto de Corte = 0.5",
       x = "probtest", 
       y = "Resultado")+
  theme_calc()+scale_fill_calc()
probtest3
...
cartllev$error<-cartllev$resultado-cartllev$pronostico
cartllev$error = as.factor(cartllev$error)
probtest4 = ggplot(cartllev,aes(probtest,resultado, color=error))+
  geom_point()+
  geom_vline(xintercept=corte)+
  labs(title = "Precisión y Exhaustividad",
       subtitle = "Punto de Corte = 0.75",
       x = "probtest", 
       y = "Resultado")+
  theme_calc()+scale_fill_calc()
probtest4
...
cartllev$error<-cartllev$resultado-cartllev$pronostico
cartllev$error = as.factor(cartllev$error)
probtest5 = ggplot(cartllev,aes(probtest,resultado, color=error))+
  geom_point()+
  geom_vline(xintercept=corte)+
  labs(title = "Precisión y Exhaustividad",
       subtitle = "Punto de Corte = 0.8",
       x = "probtest", 
       y = "Resultado")+
  theme_calc()+scale_fill_calc()
probtest5
...
cartllev$error<-cartllev$resultado-cartllev$pronostico
cartllev$error = as.factor(cartllev$error)
probtest6 = ggplot(cartllev,aes(probtest,resultado, color=error))+
  geom_point()+
  geom_vline(xintercept=corte)+
  labs(title = "Precisión y Exhaustividad",
       subtitle = "Punto de Corte = 0.95",
       x = "probtest", 
       y = "Resultado")+
  theme_calc()+scale_fill_calc()
probtest6
...
# Arrange the plots
combined_probtest = grid.arrange(probtest1, probtest2, probtest3, probtest4, probtest5, probtest6, nrow = 2, ncol = 3)
...
fprs<-unlist(curvaROC_valid@x.values)
tprs<-unlist(curvaROC_valid@y.values)
...
library("gains")
#LIFT ACUMULADO
results_test = as.data.frame(data_test$resultado
plot.gains(probtest,results_test)
#solo lift
